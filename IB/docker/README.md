...
Собираем образ с помощью Dockerfile и запускаем контейнер на основе нашего собранного образа. Перед сборкой немного изменил Dockerfile указав явные пути до скрипта внутри контейнера. Видим, что образ успешно собрался и контейнер запущенный на основе этого образа обработал наш скрипт и вывел его результат в консоль. Можем понять что это результат работы скрипта из контейнера, т.к. ОС указана Alpine Linux именно так, которая используется в контейнере bash.
