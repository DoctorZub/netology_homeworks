...
Почему то просто команда*curl* на 69 порт не срабатывал. Поэтому доступность tftp сервера проверял либо командой *tftp*, либо запросами к серверу через тот же *curl*  
...
Ниже можно увидеть пакеты с флагом FIN, что говорит нам, что это пакеты о закрытии TCP соединения.  

Для tcp-порта 22:
![22](https://github.com/DoctorZub/netology_homeworks/blob/main/img/IB/tcp_80.jpg)
